...
class SqitchRunner extends EventEmitter {
  ...
  runNative(command, args, projectDir) {
    ...
    proc.stdout.on('data', (data) => {
      this.emit('log', { type: 'stdout', text: data.toString() });
    });

    proc.stderr.on('data', (data) => {
      this.emit('log', { type: 'stderr', text: data.toString() });
    });

    proc.on('close', (code) => {
      if (code === 0) {
        this.emit('log', { type: 'success', text: `Command 'sqitch ${command}' finished successfully.` });
  ...
  runSimulated(command, args, projectDir) {
    ...
    if (command === 'init') {
      ...
      // Actually create demo files if missing
      this.initDemoProjectFiles(projectDir, projName, engine);
    } else if (command === 'status') {
      steps.push(
        { text: `# On database db:pg://postgres:postgres@localhost:5432/app_db`, delay: 200 },
      ...
      steps.push(
        { text: `Deploying changes to db:pg://postgres:postgres@localhost:5432/app_db`, delay: 200 },
      ...
      steps.push(
        { text: `Reverting changes from db:pg://postgres:postgres@localhost:5432/app_db`, delay: 200 },
        { text: `- audit_logs .. ok`, delay: 400 },
      ...
      steps.push(
        { text: `Verifying database changes...`, delay: 200 },
