...
class SqitchPlanParser {
  ...
  /**
   * Parse INI style sqitch.conf into structured object with robust regex
   */
  static parseConfigContent(content) {
    ...
    for (const line of lines) {
      ...
      const secMatch = trimmed.match(/^\[\s*([a-zA-Z0-9_-]+)(?:\s+(?:"([^"]+)"|'([^']+)'|([^\s\]]+)))?\s*\]$/);
      if (secMatch) {
        ...
        if (category === 'target' && subName) {
          ...
          if (!config.target[subName]) config.target[subName] = {};
